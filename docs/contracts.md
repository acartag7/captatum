...
when an untrusted sidecar shares the proxy's socket address. A direct request
...
- **Hosted remote server runtime**: Streamable HTTP `/mcp` + gateway OAuth,
  ...
  an ingress/egress-deny NetworkPolicy; neither shares the gateway/cloudflared
  Pod. Default state is
